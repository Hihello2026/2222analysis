...
# 1. Institutional Interface Setup
...
st.markdown("Portfolio Performance vs. TASI Benchmark")

# 2. Comprehensive Moat Portfolio (Updated with ELM & Arabian Drilling)
moat_assets = {
    ...
    '4007.SR': {'name': 'Al Hammadi', 'moat': 'Strategic Healthcare'},
...
def get_benchmarked_data(symbols, start, end):
    try:
        # Download Portfolio Assets + TASI Index (^TASI.SR)
...
if start_date < end_date:
    ...
    if not assets_data.empty:
        ...
        S = risk_models.sample_cov(assets_data)
        ...
        tasi_cum_ret = (1 + tasi_daily_ret).cumprod()

        # 5. Visualization
        st.subheader("Cumulative Returns: Portfolio vs. TASI")
        comparison_df = pd.DataFrame({
            'Portfolio': portfolio_cum_ret,
            'TASI Index': tasi_cum_ret
        })
        ...
        # TASI Metrics Calculation
        tasi_ann_ret = (tasi_cum_ret.iloc[-1] ** (252/len(tasi_data))) - 1
        tasi_ann_vol = tasi_daily_ret.std() * np.sqrt(252)
        tasi_sharpe = (tasi_ann_ret - 0.04) / tasi_ann_vol

        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric("Portfolio Return", f"{p_ret:.2%}", f"{(p_ret - tasi_ann_ret):.2%}")
        with c2:
            st.metric("Portfolio Volatility", f"{p_vol:.2%}", f"{(p_vol - tasi_ann_vol):.2%}", delta_color="inverse")
        with c3:
            st.metric("Portfolio Sharpe", f"{p_sharpe:.2f}", f"{(p_sharpe - tasi_sharpe):.2f}")

        # Allocation Table
        st.markdown("---")
        st.subheader("Optimized Allocation Weights")
        alloc_df = pd.DataFrame([
            {"Asset": k, "Weight": f"{v:.2%}", "Strategy": next((val['moat'] for key, val in moat_assets.items() if val['name'] == k), "")}
            for k, v in clean_weights.items() if v > 0
        ])
        st.table(alloc_df)
